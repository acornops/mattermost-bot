...
- Created and reopened issue alerts include a `Run Triage` action when public callback configuration is available. The callback verifies the acting Mattermost user, reloads the issue, links to recent AcornOps target chat activity when present, or creates a read-only Kubernetes cluster session and streams it into a registered Mattermost thread. Resolved alerts do not include the action.
...
- Created and reopened issue alerts include `Run Triage` when interactive callbacks are configured. The callback reloads the issue, checks recent target chat activity, and either links to the existing AcornOps cluster chat or creates a read-only cluster session and streams its console-equivalent triage prompt in a new Mattermost thread.
...
- `POST /mattermost/actions` handles interactive Mattermost button callbacks. Workspace selection, target selection, and issue triage verify the action secret and acting Mattermost user. Issue triage uses the target issues and recent chat activity APIs before linking to AcornOps or creating a threaded run. The callback returns a quiet Mattermost-compatible HTTP 200 response and posts one visible bot result message in the main conversation.

...
- On 2026-07-23, started B25: a standalone public Mattermost integration demo owned by this repository but hosted on the existing demo VM. Added a fail-closed four-service demo Compose stack, protected/idempotent seed flow, k3s Traefik EndpointSlice/Ingress adapter, public/backend health and real `!help` verification, safe down/reset behavior, shared-host deployment lease, commit-addressed activation with automatic application rollback, Task entrypoints, and a manual GitHub Actions deployment workflow. The read-only VM preflight found 15 GiB RAM with about 12 GiB available, 47 GiB free disk, low load, healthy k3s, and a deployed AcornOps release. Static demo verification and `task validate` pass with all 177 tests. Live rollout still requires the paired demo-infra client registration, DNS, GitHub environment secrets, and acceptance matrix.

...
- On 2026-07-23, deployed the B25 standalone public Mattermost integration demo to the existing demo VM. The bot repository owns the fail-closed four-service Compose stack, protected/idempotent seed, k3s Traefik adapter, shared-host lease, commit-addressed promotion/rollback, operations, and manual workflow; demo-infra owns the VM, public edge, external-client registration, and matching lease. Mattermost, the bot, both databases, public TLS, a real `!help` round trip, bot-to-AcornOps readiness, idempotent redeployment, and private-only 8065/8077 bindings pass live. A 54 MiB recovery backup preserved the exact application containers and bindings, and the post-backup smoke passed. `task validate` passes with all 177 tests. B25 stays active for the remaining interactive account-link, actions, workflow, approval, and signed-webhook matrix.
- On 2026-07-23, standardized the seeded Mattermost developer password on `devpass`, matching the AcornOps local development identity. Because Mattermost otherwise requires eight characters, the local-only Compose profile explicitly lowers the minimum to seven. Existing persistent stacks receive the new password through the next `task local-up`; explicit environment overrides remain supported. A repeated-start smoke exposed that an unchanged bot container could retain a dead WebSocket after Mattermost recreation, so `local-up` now force-recreates the bot after Mattermost becomes healthy.
- On 2026-07-23, completed B24: a task-managed seeded local Mattermost bot environment. `task local-up` now starts native-architecture Mattermost 11.7.0, separate Mattermost and bot Postgres databases, and the bot; it idempotently seeds `dev@acornops.local`, `csit-lab/chatops-lab`, `@acorn-ops-bot`, memberships, and an ignored mode-0600 bot token by default. `local-down` preserves state, while `local-reset` removes only this Compose project's volumes and token. A clean reset/reseed, repeated seed, preserved restart, all service health checks, AcornOps health, and a real `@acorn-ops-bot !help` WebSocket round trip passed. Final `task validate` passed with Compose validation, lint, build, and all 177 tests. The stack is currently running at `http://localhost:8065` for the remaining interactive account-link/workflow/webhook smoke matrix.
- On 2026-07-23, a production-readiness contract sweep found that the bot still consumed the pre-Workflow-V2 `policy`, per-step context grants, `targetBinding`, and `@cluster[...]` shape. It now prefers the control plane's current `capabilityPolicy`, `resourceRequirements`, `prompt`, and `@target[...]` contract while retaining a legacy-definition fallback. Declared inputs are preserved in bounded message content, session creation sends the exact current context grants, and persisted launch-target metadata keeps follow-ups stable across global target changes. Final `./init.sh` passed with harness verification, lint, build, and all 175 tests; live Mattermost/AcornOps smoke remains outstanding.
- Earlier on 2026-07-23, aligned launch and follow-up bodies with the strict `content` plus `clientRequestId` shape and removed stale fields from message requests. That pass incorrectly assumed inputs and resource bindings were session-owned; the production-readiness entry above supersedes that interpretation. Focused tests and `./init.sh` passed at that stage with 172 tests.
- On 2026-07-19, reviewed all 18 commits ahead of `origin/main`. Fixed webhook retry loss by releasing in-memory and Postgres event reservations when Mattermost delivery fails, and bound `Run Triage` tokens to the originating alert channel to reject cross-channel replay. Focused verification passed with 37 tests; final `./init.sh` passed with harness verification, lint, build, and all 172 tests. Mattermost readiness still failed because `localhost:8065` was not listening. The npm advisory lookup was not run because execution policy denied external transmission of repository dependency metadata.
- On 2026-07-19, resolved all five review findings against B23. All Mattermost buttons now carry expiring HMAC tokens while the action key remains server-side; approval dialogs use a separate short-lived token purpose. Initial workflow launches persist and reuse the source-post session reservation and serialize concurrent delivery within one process. Workflow cursors/dedupe state advance only after Mattermost delivery, active executions survive repeated reconnect/poll cycles, and terminal posts are retried before cleanup. AcornOps approval success is now independent of Mattermost post-update success, with bounded UI retries and accurate logging. Focused verification passed with 119 tests, full `npm test` passed with 170 tests, and final `./init.sh` passed with harness verification, lint, build, and all 170 tests. Existing raw-context buttons become invalid after deployment; Mattermost documents action context as confidential, so no emergency key rotation is required solely for the previous shape.
- On 2026-07-18, completed B23: workspace-wide `!issues`; generic unknown handling for removed `!findings`/`!investigations`; all eligible workflow modes with post-derived `clientRequestId`; aggregate multi-step execution replay/final-attempt following; and confirmed, exact-user Mattermost **Approve**/**Reject** actions with signed dialog state and console fallback. Full `npm test` and final `./init.sh` passed with 160 tests. Mattermost and AcornOps were unavailable on `localhost:8065`/`localhost:8081`, so live issue/workflow/approval smoke remains outstanding. Generated report delivery and restart-time follower recovery remain deferred.
- On 2026-07-15, completed an iterative local-change/code/documentation/harness audit. Fixed `!` command guidance, current-state and live-smoke drift, added harness JSON/command-reference checks, ignored generated `tmp/` renders, and published wiki commit `366f26a` documenting the Mattermost issue-alert `Run Triage` action. Baseline and final `./init.sh` passed with all 148 tests; focused command/server verification passed with 86 tests.
- On 2026-07-13, added `Run Triage` to created/reopened Mattermost issue alerts. The action reloads the issue, checks recent AcornOps target chat activity, links to an existing console session when present, or creates a read-only cluster session and streams it in a registered Mattermost thread. Final `./init.sh` passed with 148 tests; live signed-alert delivery/action smoke remains outstanding.
...
- Current implementation includes eligible read-only, read-write, and approval-gated workflow discovery and threaded execution. `!workflow run <number|id> [key=value...]` binds the selected target, sends a stable per-post `clientRequestId`, follows the returned aggregate `executionId` across every step with replay deduplication, and loads final output from the latest attempt.
...
- `!resources` uses the selected target. `!issues` lists workspace-wide issues with contract filters; removed `!findings` and `!investigations` receive the generic unknown-command response.
- `!chat new [title]` creates a read-only troubleshooting thread. `!chat new --write [title]` checks effective workspace permission and creates a persisted read-write thread. Approval requests offer signed, exact-user confirmation actions when callbacks are configured and otherwise link to the AcornOps console; approval resolution plus the final run result remain in the thread.
- `!workflows` lists every eligible active workflow with mode and approval labels. `!workflow run <number|id> [key=value...]` launches one with exact context grants and target bindings, then aggregate execution following keeps every step and plain follow-up in the dedicated workflow thread.

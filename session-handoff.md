...
- On 2026-07-23, aligned the bot with the strict integrated Workflow session-message contract. Launches and follow-ups now send exactly `content` and the required post-derived `clientRequestId`; Workflow inputs, grants, and target bindings remain fixed at session creation. Current docs no longer claim that message requests resend launch fields or that approvals are browser-only. Focused tests and final `./init.sh` passed with harness verification, lint, build, and all 172 tests. Live Mattermost/AcornOps smoke remains outstanding.

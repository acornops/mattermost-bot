...
- Repository root: this checkout; no machine-specific path is required.
...
- `!workflows` lists every eligible active workflow with current `capabilityPolicy` read-only/read-write and approval labels. `!workflow run <number|id> [key=value...]` validates declared inputs, uses exact `capabilityPolicy.contextGrants`, checks `resourceRequirements`, renders the inputs into launch content, and materializes the selected target as `@target[...]` without a separate write flag.
- Accepted workflow launches create a Mattermost root post exactly like `**Workflow launched: Cluster triage**`. Every initial and follow-up message carries a post-derived `clientRequestId`; initial launches also persist and reuse a bounded source-post-to-session reservation. The aggregate replayable execution stream discovers each step and persists its `executionId`, current `runId`, and replay cursor only after required Mattermost delivery succeeds.
- Workflow session creation carries only the workspace and exact context grants. Initial and follow-up message bodies contain only `content` and `clientRequestId`; declared launch inputs and typed target references live in content, and AcornOps resolves resource bindings per execution. The bot persists launch-target metadata locally so a workflow thread remains on its original target after global target selection changes.
...
- Troubleshooting and workflow approval requests show user-bound **Approve**/**Reject** buttons when callback configuration is complete. Button context contains only an expiring signed token; `MATTERMOST_ACTION_SECRET` remains server-side. Both choices open confirmation-only dialogs with separate short-lived signed immutable state before the bot calls AcornOps. Successful or settled decisions retry button removal, and a Mattermost update failure cannot misreport the recorded AcornOps decision. The AcornOps console remains the fallback when callbacks are unavailable.
...
- Chat timing environment variables are `CHAT_RUN_POLL_ATTEMPTS`, `CHAT_RUN_POLL_INTERVAL_MS`, `RUN_STREAM_RECONNECT_ATTEMPTS`, `RUN_STREAM_RECONNECT_DELAY_MS`, `RUN_STREAM_FALLBACK_POLL_INTERVAL_MS`, `RUN_STREAM_FALLBACK_POLL_MAX_MS`, and `WORKFLOW_FOLLOW_RETRY_DELAY_MS`. A workflow that remains active after one bounded fallback window retains its active-thread state and starts another reconnect/poll cycle.
...
- Updated the current runtime, API inventory, README, decision, progress, and handoff artifacts to remove the stale request-shape and browser-only approval claims. Machine-specific current-state paths were replaced with portable repository references.
- Focused workflow contract tests passed, and final `./init.sh` passed with harness verification, lint, build, and all 175 tests. Live Mattermost/AcornOps smoke remains the release-time integration gate.
...
- Completed: Replaced raw action-secret attachment context across workspace, target, issue-triage, and approval buttons with purpose-bound expiring HMAC tokens; added separate short-lived approval-dialog tokens and suppressed all unsigned actions. Persisted and reused initial workflow post-to-session reservations, including single-process concurrent delivery serialization. Moved workflow cursor and approval dedupe commits after successful Mattermost delivery, retained non-terminal workflows across repeated reconnect/poll cycles, and preserved active state until terminal result delivery. Separated AcornOps decision errors from Mattermost post-update errors and added bounded post-update retries.
- Verification run: Focused action, command, server, context, Postgres, and follower suites passed with 119 tests. Full `npm test` passed with 170 tests. Final `./init.sh` passed with harness verification, lint, build, and all 170 tests. Live smoke could not run because Mattermost and AcornOps were not listening on `localhost:8065` and `localhost:8081`.
- Known risks: Existing buttons emitted by commit `9025340` use the former raw-secret context shape and become invalid after deployment; Mattermost documents that context as confidential, so no emergency rotation is required solely for that reason. Multi-replica launch serialization, restart-time follower recovery, generated report delivery, and live Mattermost/AcornOps smoke remain separate follow-ups.
...
- Updated workflow launch and follow-up messages to render the selected target reference and, at that time, resend explicit target id/type bindings. The 2026-07-23 strict-contract hardening removed those unsupported fields; the later Workflow V2 correction now emits `@target[...]` in content and lets AcornOps resolve the binding per execution.

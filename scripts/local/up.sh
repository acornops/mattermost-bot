...
local_compose up -d --build --wait --no-deps --force-recreate mattermost-bot
...
if [[ "${SEED_MATTERMOST_DATA:-true}" == "true" ]]; then
  printf 'Seeded login: %s / %s\n' "${LOCAL_MATTERMOST_USER_EMAIL:-dev@acornops.local}" "${LOCAL_MATTERMOST_USER_PASSWORD:-devpass}"
